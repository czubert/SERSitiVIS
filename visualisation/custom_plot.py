...
def show_plot(df, plots_color, template, display_opt, key):
    ...
    if display_opt == SINGLE:
        ...
        if spectra_conversion_type == RAW:
            ...
            deg, window = adjust_spectras_window_n_degree()
        ...
        if spectra_conversion_type == RAW:
            ...
            if adjust_plots_globally == 'all':
                ...
                with st.beta_expander("Customize your chart"):
                    vals = {col: adjust_spectras_window_n_degree(col) for col in df.columns}
...
def bwtek_vis_options(df, plots_color, template):
    ...
    show_plot(df, plots_color, template, display_opt=display_options_radio, key=None)
